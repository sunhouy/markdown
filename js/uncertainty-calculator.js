...
function processUncertaintyDigits(uncertainty) {
    ...
    return Math.ceil(uncertainty / factor) * factor;
}
...
function processResultDigits(value, uncertainty) {
    ...
    if (uncertaintyStr.includes('.')) {
        decimalPlaces = uncertaintyStr.split('.')[1].length;
    } else {
        if (Math.abs(uncertainty) < 1) {
            const exp = Math.floor(Math.log10(Math.abs(uncertainty)));
            decimalPlaces = -exp;
        } else {
            decimalPlaces = 0;
        }
...
function generateUncertaintyResult(typeB, values) {
    ...
    const processedUncertainty = processUncertaintyDigits(combined);
    const processedMeanValue = processResultDigits(mean, processedUncertainty);
    
    // 获取不确定度的小数位数，用于格式化测量值
    const uncDecimalPlaces = getDecimalPlaces(processedUncertainty);
    // 格式化测量值，补足到与不确定度相同的小数位数
    const formattedMean = processedMeanValue.toFixed(uncDecimalPlaces);
    ...
    // 不确定度修约过程
    const mag = Math.floor(Math.log10(combined));
    const norm = combined / Math.pow(10, mag);
    const firstDigit = Math.floor(norm);
    const sigDigits = (firstDigit >= 1 && firstDigit <= 3) ? 2 : 1;
    const factor = Math.pow(10, mag - (sigDigits - 1));
    const rawUnc = combined;
    const ceilUnc = Math.ceil(rawUnc / factor) * factor;
    
    // 测量结果修约过程
    const uncStr = processedUncertainty.toString();
    let alignDecimals = 0;
    if (uncStr.includes('.')) alignDecimals = uncStr.split('.')[1].length;
    else if (Math.abs(processedUncertainty) < 1) alignDecimals = -Math.floor(Math.log10(Math.abs(processedUncertainty)));
...
$$\\bar{x} = \\frac{1}{n}\\sum_{i=1}^{n} x_i = \\frac{${values.map(v => formatNum(v)).join(' + ')}}{${n}} = ${formatNum(sum)} \\div ${n} = ${formatNum(mean)}$$
...
- 科学计数法：${norm.toFixed(6)} × 10^{${mag}}
- 首位数字：${firstDigit}
- 因为首位数字 ${firstDigit} ${firstDigit <= 3 ? '≤3' : '>3'}，故取 ${sigDigits} 位有效数字
- 修约因子：$10^{${mag - (sigDigits - 1)}} = ${factor}$
- 向上取整：$\\lceil ${combined} / ${factor} \\rceil \\times ${factor} = ${Math.ceil(combined/factor)} \\times ${factor} = ${ceilUnc}$

处理后不确定度：${processedUncertainty}

## 8. 测量结果的修约（与不确定度末位对齐）

- 平均值原始值：${formatNum(mean)}
- 不确定度已修约为 ${processedUncertainty}，其小数位数为 ${alignDecimals}
- 将平均值修约到 ${alignDecimals} 位小数（银行家舍入）

银行家舍入规则：四舍六入五成双  
- 将 ${mean} 乘以 $10^{${alignDecimals}}$ 得 ${mean * Math.pow(10, alignDecimals)}
- 取整后按规则处理

得到修约后测量结果数值：${processedMeanValue}  
最终格式化（补足到 ${alignDecimals} 位小数）：${formattedMean}

## 9. 相对不确定度

$$U_r = \\frac{u_c}{\\bar{x}} \\times 100\\% = \\frac{${processedUncertainty}}{${processedMeanValue}} \\times 100\\% = ${(relative*100).toFixed(6)}\\%$$

保留两位有效数字：${processedRelative}

## 10. 最终结果表示

$$${formattedMean} \\pm ${processedUncertainty}$$
...
### 相对不确定度
$$U_r = \\frac{u_c}{\\bar{x}} \\times 100\\%$$

### 间接测量不确定度传递公式示例（圆环体积）
对于圆环体积 $V = \\frac{\\pi}{4} (D_2^2 - D_1^2) H$，其相对不确定度为：

$$\\frac{\\Delta_V}{V} = \\sqrt{\\left( \\frac{2D_2}{D_2^2 - D_1^2} \\Delta_{D_2} \\right)^2 + \\left( \\frac{2D_1}{D_2^2 - D_1^2} \\Delta_{D_1} \\right)^2 + \\left( \\frac{\\Delta_H}{H} \\right)^2}$$
`;
...
function showUncertaintyCalculator() {
    ...
    const dialogHTML = `
        <div class="modal-overlay" id="uncertaintyCalculatorOverlay" style="display: flex; align-items: center; justify-content: center; position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(0,0,0,0.5); z-index: 10000;">
            <div class="modal" style="background: white; border-radius: 8px; padding: 20px; width: 90%; max-width: 500px; max-height: 80vh; overflow-y: auto;">
                <div class="modal-header" style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
                    <h2>不确定度计算器</h2>
                    <button class="modal-close-btn" id="closeUncertaintyCalculator" style="background: none; border: none; font-size: 20px; cursor: pointer;">
                ...
                <div class="modal-form">
                    <div class="form-group" style="margin-bottom: 15px;">
                        ...
                        <input type="number" id="typeBInput" step="0.0001" placeholder="请输入仪器误差" style="width: 100%; padding: 8px; border: 1px solid #ddd; border-radius: 4px;">
                    </div>
                    
                    <div class="form-group" style="margin-bottom: 15px;">
                        <label for="measurementCount" style="display: block; margin-bottom: 5px;">测量次数:</label>
                        <input type="number" id="measurementCount" min="2" value="5" style="width: 100%; padding: 8px; border: 1px solid #ddd; border-radius: 4px;">
                    </div>
                    ...
                    <div class="modal-actions" style="display: flex; justify-content: space-between;">
                        <button class="modal-btn secondary" id="cancelUncertaintyBtn" style="padding: 8px 16px; border: 1px solid #ddd; border-radius: 4px; background: #f5f5f5; cursor: pointer;">取消</button>
...
function updateMeasurementInputs() {
    ...
    for (let i = 1; i <= count; i++) {
        ...
        inputGroup.innerHTML = `
            <label for="measurement${i}" style="display: inline-block; width: 60px;">测量${i}:</label>
            <input type="number" id="measurement${i}" step="0.0001" placeholder="请输入值" style="width: calc(100% - 70px); padding: 6px; border: 1px solid #ddd; border-radius: 4px;">
        `;
